...
function actualizarDashboard(filtros = {}) {
  ...
  // Calcular totales
  const totalPiezas = ventasFiltradas.reduce((sum, venta) => sum + venta.piezas, 0);
  const metaTotal = metas.reduce((sum, meta) => sum + meta.meta, 0);
...
function actualizarGraficoAvance(ventasFiltradas) {
  ...
  const datos = sucursales.map(sucursal => {
    ...
    const piezasVendidas = ventasSucursal.reduce((sum, v) => sum + v.piezas, 0);
